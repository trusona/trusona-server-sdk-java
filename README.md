...
[![Maintainability](https://api.codeclimate.com/v1/badges/5959a0775d2ada2d326f/maintainability)](https://codeclimate.com/github/trusona/trusona-server-sdk-java/maintainability)
...
```groovy
dependencies {
  compile "com.trusona:trusona-sdk:4.2.0"
}
```
...
[^1]: You must provide at least one field that would allow Trusona to determine which user to authenticate. The identifier fields are `deviceIdentifier`, `truCode`, `userIdentifier`, and `email`.
...
```java
Trusona trusona = trusona = new Trusona(token, secret, TrusonaEnvironment.EU_PRODUCTION);
```
